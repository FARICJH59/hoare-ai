...
export async function POST(request: NextRequest) {
  ...
  if (!stripeSecretKey || !priceId) {
    return NextResponse.redirect(new URL("/dashboard?billing=missing-config", request.url));
  }
  ...
  if (!session.url) {
    return NextResponse.redirect(new URL("/dashboard?billing=checkout-failed", request.url));
  }
