...
import { energyRouter, getEnergyHealth } from "./energy";
import { billingRouter, getBillingHealth } from "./billing";
...
async function buildGovernanceStatus() {
  ...
  const energy = await getEnergyHealth();
  const billing = await getBillingHealth();
...
export function createApp(): express.Application {
  ...
  app.get("/health", async (_req: Request, res: Response) => {
    ...
    res.json({
      ...
      dependencies: {
        ...
        energy: governance.energy.reachable ? "configured" : "not-configured",
        billing: governance.billing.reachable ? "configured" : "not-configured",
      },
      governance,
  ...
  app.get("/metrics", async (_req: Request, res: Response) => {
    ...
    metrics.gauge("hoare_energy_subsystem_reachable", governance.energy.reachable ? 1 : 0);
    metrics.gauge("hoare_billing_subsystem_reachable", governance.billing.reachable ? 1 : 0);
